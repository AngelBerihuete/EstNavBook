...
En las siguientes secciones iremos desarrollando los elementos básicos de un análisis exploratorio, calculando medidas que nos ayuden a interpretar de forma adecuada el conjunto de datos analizados. Para ello utilizaremos varios conjuntos de datos que podrán ser descargados de 

## Problema

- Los precios (en euros) de un conjunto de botellas de vino en cierta sección de un centro comercial son:
...
- Un **análisis exploratorio de datos** implica el uso de los elementos numéricos y gráficos que describan lo más exhaustivamente posible un conjunto de datos.

- Un **análisis exploratorio de datos** deberá poner de manifiesto los aspectos más relevantes de la distribución de los datos: regularidades, datos atípicos, especificidades, relaciones, etc.
...
Tabla de frecuencias del factor `r colnames(fluoruro)[2]` que tiene niveles `r unique(fluoruro$tipo)`
...
Tabla de frecuencias de la variable cuantitativa continua `r colnames(fluoruro)[3]`
...
<font color="red">¡Cuidado! Longitud de intervalos distinta.</font>
...
![Fuente: Verne (El País)](grafico_erroneo1.jpg)


![Fuente: Verne (El País)](grafico_erroneo2.jpg)


![Fuente: Verne (El País)](grafico_erroneo3.jpg)

2. No se mantiene la escala en todo el gráfico

![Fuente: Verne (El País)](grafico_erroneo4.png)

![Fuente: Verne (El País)](grafico_erroneo4bis.png)


Pueden encontrar toda esta información sobre la manipulación de gráficos en

- https://verne.elpais.com/verne/2015/01/23/articulo/1422014607_223837.html

y en la cuenta de Twitter de Kiko Llaneras @kikollan
...
### La media aritmética de variable cuantitativa discreta
...
### La media aritmética en variables continuas agrupadas en intervalos

Para **variables continuas** agrupadas en intervalos tendremos que obtener la **marca de clase**
...
### Cálculo de la Mediana en **variables discretas**
...
### La mediana en variables discretas

```{r, echo=FALSE}
knitr::kable(tab_precios, format = 'html', escape = FALSE, digits = 4, padding = 2)
```

Si $N$ es impar, entonces la mediana es el valor que ocupa la posición
...
Si $N$ es par, entonces la mediana es la media de los valores que ocupan la posión $\frac{78}{2} = 39$ y $\frac{78}{2}+1 = 40$. Mirando en la tabla

$$M_e = \frac{52+52}{2} = 52$$

### La mediana en variables continuas

```{r, echo=FALSE}
knitr::kable(tab_concen[,1:5], format = 'html', escape = FALSE, digits = 4, padding = 2)
```

El primer $N_i$ que es mayor o igual a $\frac{50 \cdot 79}{100} = 39.5$  es $N_1 = 45$, entonces $M_e$ está dentro del intervalo 
...
-   La absoluta de una distribución es el valor que más veces se repite.
...
### La moda para variables discretas

```{r, echo=FALSE}
knitr::kable(tab_precios, format = 'html', escape = FALSE, digits = 4, padding = 2)
```

El valor que más se repite es $M_o = 42$
...
-   La media es la mejor de las medidas de representación, pues la moda es bastante inestable y un pequeño cambio en las observaciones puede afectarle mucho, mientras que la mediana es insensible al tamaño de los datos.

-   La media es sensible a las alteraciones de los datos, al tamaño de éstos y si se conocen las medias de dos conjuntos de datos, basta con saber los tamaños de ambos grupos para calcular la media global.
...
-   Se llaman medidas de posición o de orden $k$ a aquellas que dividen a la distribución en $k$ partes, de tal forma que en cada una de esas partes haya el mismo número de elementos.
...
El cálculo de los percentiles será suficiente para calcular tanto los deciles como los cuartiles. 
...
### Para datos agrupados en intervalos utilizaremos la fórmula:
...
-   La caja representa el 50% central de la distribución, la línea
    situada en el interior de la caja es la mediana, mientras que la
    cruz se corresponde con la media.
...
-   Los candidatos a valores anómalos se etiquetan como **atípicos** y se
    encuentran fuera del intervalo $(Q_1 - 1.5 \cdot R_I,Q_3 + 1.5 \cdot R_I)$:
...
-   Observe que cuando la distribución es simétrica coinciden la media y la mediana.

-   Si la distribución tiene además forma de campana, la media y la mediana se aproximan a la moda.
...
```{r, echo=FALSE}
skew_tipo <- skewness_tipo <- data_skewness %>% 
  group_by(dist) %>%
  summarise(skewness(valor, type = 3))
```

La asimetría de las anteriores distribuciones son `r skewness_tipo[[2]][1]`, `r skewness_tipo[[2]][2]` y `r skewness_tipo[[2]][3]`, respectivamente.
...
kurt_dist <- data_kurt %>% 
  group_by(dist) %>%
  summarise(kurtosis(valor, type = 3))
```

La asimetría de las anteriores distribuciones son `r kurt_dist[[2]][1]`, `r kurt_dist[[2]][2]` y `r kurt_dist[[2]][3]`, respectivamente.


# Tipificación

- A veces la distribución presenta muchas irregularidades, como asimetrías acentuadas, valores extremos, etc. En estos casos es recomendable efectuar una transformación que la haga más regular

- Un caso particular de transformación es la **tipificación**.
